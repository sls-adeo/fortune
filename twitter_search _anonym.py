...
def write_tweets(tweets, filename, filename2):
    ...
    with open(filename2, 'a') as f:
        for tweet in tweets:
            ...
            # json.dump(tweet.created_at, f)

            if (not tweet.retweeted): # and ('RT @' not in tweet.text):
                dttweet = tweet.created_at
                f.write(dttweet.strftime("%d-%m-%Y %H:%M:%S") + '\t' + tweet.text + '\n')
...
def main():
    ...
    ''' search variables: '''
    search_phrases = ['@adeo', '@leroymerlin',
                      '@kbane', '@bricoman', '@zodio'] 
    ...
    for search_phrase in search_phrases:
        ...
        txt_file = json_file_root + '_' + day + '.txt'
        ...
        while dt.datetime.now() < end:
            ...
            if tweets:
                write_tweets(tweets, json_file, txt_file)
                exitcount = 0
            else:
                exitcount += 1
                if exitcount == 3:
